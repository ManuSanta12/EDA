        ...
        */
        ...
        private T remove(T value, Node<T> node) {
            ...
            if (node.numberOfChildren() == 0) {
                ...
                Node<T> gt = node.getChild(index+1);
                Node<T> lt = this.getLowestNode(gt);
                T replaceValue = this.removeLowestValue(lt);
                node.addKey(replaceValue);
                if (lt.parent != null && lt.numberOfKeys() < minKeySize) {
                    this.combined(lt);
                }
                if (lt.numberOfChildren() > maxChildrenSize) {
                    this.split(lt);
                }
            }
